...
class BeerTrainer:
def **init**(self, data_file=DATA_FILE, beer_db_file=BEER_DB_FILE):
...
def load_beer_db(self, file_path):
    if os.path.exists(file_path):
        ...
        df = df.fillna("")
        beer_dict = df.set_index("beer_id").to_dict(orient="index")
        return beer_dict
    else:
...
def get_top_beers(self, top_n=5):
    ...
    top_beer_ids = [beer_id for beer_id, _ in sorted_beers[:top_n]]
    results = []
    for beer_id in top_beer_ids:
